    ...
    if config.HYPERPARAM_TUNING:
        ...
        tasks = [
            (
                ...
                np.random.choice(getattr(config, "ADAM_CHOICES", [True])),
                np.random.choice(getattr(config, "SCHEDULE_CHOICES", ["none"])),
...
def main() -> None:
    ...
    if config.HYPERPARAM_TUNING:
        ...
        best_adam = bool(npz["adam"]) if "adam" in npz.files else None
        best_sched = str(npz["schedule"]) if "schedule" in npz.files else None
        best_weighted = bool(npz["weighted_bce"]) if "weighted_bce" in npz.files else getattr(config, "USE_WEIGHTED_BCE", False)
        print(f"[Loaded] Best params from -> {config.SAVE_BEST}")
        print(
            f"[BEST] lambda={best_lambda:.3e}, gamma={best_gamma:.3e}, thr={best_thr:.3f}, "
            f"ACC={val_acc:.4f}, P={val_prec:.4f}, R={val_rec:.4f}, F1={val_f1:.4f}, "
            f"adam={best_adam}, schedule={best_sched}, weighted_bce={best_weighted}"
        )
        setattr(config, "USE_WEIGHTED_BCE", best_weighted)
